...
function seedAgents(): Agent[] {
  return [
    {
      id: 'agent-orchestrator', name: 'Orchestrator', domain: 'CORE',
      criticity: 'critique', status: 'active', version: '1.2.0',
      description: 'Central orchestrator — coordinates all agent workflows and state transitions.',
      tags: ['orchestration', 'core', 'workflow'], dependencies: [],
      capabilities: [
    ...
    {
      id: 'agent-aqa', name: 'AQA Agent', domain: 'QA',
      criticity: 'haute', status: 'active', version: '2.0.0',
    ...
    {
      id: 'agent-dev', name: 'Dev Agent', domain: 'DEV',
      criticity: 'moyenne', status: 'active', version: '1.0.0',
    ...
    {
      id: 'agent-exp', name: 'Expert Agent', domain: 'EXP',
      criticity: 'basse', status: 'draft', version: '0.5.0',
      description: 'Domain expert — provides specialized knowledge and recommendations.',
      tags: ['expert', 'knowledge'], dependencies: [],
      capabilities: [
...
function seedLogs(reader: LogReader): void {
  ...
  for (let i = 0; i < 20; i++) {
    reader.append({
      id: `log-${i}`, timestamp: new Date(Date.now() - i * 60_000).toISOString(),
      level: levels[i % levels.length], agent: agents[i % agents.length],
...
export function createCoreService() {
  ...
  doctor.setContext({
    logs: logReader.readLogs() as any,
    history: stateMachine.getHistory() as any,
