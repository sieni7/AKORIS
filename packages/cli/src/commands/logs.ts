...
import { LogReader } from '@akoris/core';
import { getProjectRoot } from '../services/project.service.js';
import { shouldOutputJSON, printJSON, title, info, warn, log, error } from '../output/format.js';
...
export const logsCommand = new Command('logs')
  ...
  .action(async (options: { lines?: string; agent?: string; since?: string }) => {
    ...
    const entries = await reader.readLogs(filter);
