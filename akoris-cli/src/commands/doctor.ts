...
import { step, success, warn, info } from "../utils/logger.js";
  ...
  .action((options) => {
    ...
    if (!existsSync(manifestPath)) {
      ...
      try {
        ...
        if (!m.project) issues.push("⚠️  Projet non défini dans MANIFEST.json");
        if (!m.owner) issues.push("⚠️  Propriétaire non défini dans MANIFEST.json");
