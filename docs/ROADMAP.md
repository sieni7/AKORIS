# Roadmap AKORIS

---

## 🔴 Urgent (Terminé) ✅

- Copyright clarifié
- Placeholder npm publié (`akoris@0.0.1`)
- README : installation marquée "prévue"
- Versioning clarifié (méthode vs écosystème)
- Nommage agents vérifié
- Disponibilité du nom AKORIS vérifiée

---

## 🟠 Court terme (En cours)

| # | Action | Effort | Statut |
|---|---|---|---|
| I6 | Badges README | 15 min | ✅ |
| I4 | CODE_OF_CONDUCT.md | 15 min | ⏳ |
| I9 | Exemple annoté state-machine.json | 30 min | ⏳ |
| I1 | Diagramme du cycle de vie | 1h | ⏳ |
| I8 | Roadmap publique | 1h | ⏳ |
| I7 | FAQ | — | ✅ (déjà existante) |
| I10 | Tableau positionnement comparatif | 1h | ⏳ |
| I2 | Section "avant/après" | 1h30 | ⏳ |

---

## 🟡 Moyen terme

| # | Action | Effort | Statut |
|---|---|---|---|
| M1 | Templates d'issues GitHub | 30 min | ✅ |
| M2 | Template PR | 20 min | ✅ |
| M3 | CI (lint, validation JSON) | 1h30 | ✅ |
| M4 | Glossaire enrichi | 1h | ✅ |
| M5 | Politique de versioning | 30 min | ✅ |
| M6 | README en anglais | 1h30 | ✅ |
| M7 | CHANGELOG enrichi | 30 min | ✅ |
| M8 | Schéma d'architecture globale | 1h | ✅ (déjà existant, référence README ajoutée) |
| M9 | Exemple de projet réel | 2h | ✅ (dogfooding — état courant, pas cas conclu) |

---

## 🟢 Long terme

- Site web / landing page
- Vidéo de présentation
- Études de cas
- Reconnaissance externe