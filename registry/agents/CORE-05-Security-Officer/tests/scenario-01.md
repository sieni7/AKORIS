...
## Objectif
Vérifier que le contrat de l'agent CORE-05 Security Officer est complet et valide.
...
3. Vérifier que `id` vaut "CORE-05".
4. Vérifier que `version` suit le format sémantique (x.y.z).
5. Vérifier que `tags` contient "core", "security", "compliance".
